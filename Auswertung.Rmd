...
CG <- subset(MA_data, MA_data$Condition == 3)
EG2 <- subset(MA_data, MA_data$Condition == 2)
EG1 <- subset(MA_data, MA_data$Condition == 1)

#AMOTIVATION
par(mfrow=c(1,2))
shapiro.test(CG$AMOTIVATION_SUM)
hist(scale(CG$AMOTIVATION_SUM))
shapiro.test(EG1$AMOTIVATION_SUM)
hist(scale(EG1$AMOTIVATION_SUM))
shapiro.test(EG2$AMOTIVATION_SUM)
hist(scale(EG2$AMOTIVATION_SUM))

#EXTERNAL_REGULATION
par(mfrow=c(1,2))
shapiro.test(CG$EXTERNAL_REGULATION_SUM)
hist(scale(CG$EXTERNAL_REGULATION_SUM))
shapiro.test(EG1$EXTERNAL_REGULATION_SUM)
hist(scale(EG1$EXTERNAL_REGULATION_SUM))
shapiro.test(EG2$EXTERNAL_REGULATION_SUM)
hist(scale(EG2$EXTERNAL_REGULATION_SUM))
```

#ANOVA Testen
```{r}
ANOVA_BENEFIT <- aov(MA_data$INTENTION_SUM ~ MA_data$Condition + MA_data$AGE_GROUP)
summary(ANOVA_BENEFIT)
t.test(MA_data$INTEGRATED_REGULATION_SUM ~ MA_data$AGE_GROUP)
